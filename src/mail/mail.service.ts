...
export class MailService {
  ...
  async sendForgotPasswordEmail(email: any, data: any) {
    ...
    let subject = 'Forget password request'
    // Email options
    const mailOptions = {
      from: `${credentials?.sender_name} <${credentials?.username}>`, // Custom name and email
      to: email,
      subject: subject,
      html: await this.forgetOtp_html(data),
    };
    const info = await transporter.sendMail(mailOptions);
    return info;
...
<body>
  <div class="container">
    ...
    <p>Best regards,<br>
    Nebras Open Finance Team</p>

    <div class="footer">
      If you have any questions, contact us at <a href="mailto:billing@nebrasopenfinance.ae">billing@nebrasopenfinance.ae</a>.
    </div>
  </div>
</body>
</html>
